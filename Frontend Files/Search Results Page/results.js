...
// Dont display the register/log in buttons if signed in already
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const old_dep_loc = JSON.parse(localStorage.getItem('dep_loc'));
    const old_arr_loc = JSON.parse(localStorage.getItem('arr_loc'));
    const old_dep_date = JSON.parse(localStorage.getItem('dep_date'));
    const old_return_date = JSON.parse(localStorage.getItem('return_date'));
    ...
    // Set the appropriate values to the text boxes
...
function displaySearchResults(results) {
    ...
    results.forEach(result => {
        ...
        resultContainer.innerHTML = `
            ...
            <h4 class="time-3">${result.returning_dep_time}</h4>
            <h4 class="time-4">${result.returning_arr_time}</h4>
            <h5 class="loc-3">${result.returning_dep_id}</h5>
            <h5 class="loc-4">${result.returning_arr_id}</h5>
            <h5 class="duration-2">${result.returning_duration}</h5>
            <h6 class="stops-2">${result.returning_stops}</h6>
            <hr class="flight-line-2">
            <div class="airline-image">
                <img src="${result.airline_image}"alt="">
            </div>
            <div class="airline-image">
                <img src="${result.returning_airline_image}" alt="">
            </div>
            <h6 class="airline-label-1">${result.airline}</h6>
            <h6 class="airline-label-2">${result.returning_airline}</h6>
...
async function buyNow(event) {
    ...
    if (event.target.classList.contains('buy-now-button')) {
        ...
        const flightDetails = {
            ...
            returning_dep_time: resultDiv.querySelector('.time-3').innerText,
            returning_arr_time: resultDiv.querySelector('.time-4').innerText,
            returning_dep_id: resultDiv.querySelector('.loc-3').innerText,
            returning_arr_id: resultDiv.querySelector('.loc-4').innerText,
            returning_duration: resultDiv.querySelector('.duration-2').innerText,
            returning_stops: resultDiv.querySelector('.stops-2').innerText,
            airline: resultDiv.querySelector('.airline-label-1').innerText,
            returning_airline: resultDiv.querySelector('.airline-label-2').innerText,
        };
        ...
        for (let i = 0; i < all_results.length; i++) {
            ...
            if (
                ...
                result.price === flightDetails.price &&
                result.returning_dep_time === flightDetails.returning_dep_time &&
                result.returning_arr_time === flightDetails.returning_arr_time &&
                result.returning_dep_id === flightDetails.returning_dep_id &&
                result.returning_arr_id === flightDetails.returning_arr_id &&
                result.returning_duration === flightDetails.returning_duration &&
                result.returning_stops === flightDetails.returning_stops &&
                result.airline === flightDetails.airline &&
                result.returning_airline === flightDetails.returning_airline
            ) {
